...
where $p(D)$ is the evidence or marginal likelihood, and often intractable quantity that acts as a normalizing constant on the product of the likelihood and prior.
...
On the other hand, Variational Inference (VI)\citep{jordan_introduction_1999,blei_variational_2017} reformulates traditional Bayesian inference as an optimization problem, where the posterior is approximated by a parametric family of distributions that is closest to the true posterior through a suitable measure. This is usually the reverse KL divergence or other measures from the family of $f$-divergence between probabiliy distributions. 
...
Here, $T_{\sharp} \mu$ is the pushforward measure of $\mu$ through map $T$, and $\CH$ is the normed function space chosen to optimize over. This minimization is implicitly achieved through the connection between the derivative of KL divergence with Stein's identity, leading to the Langevin-Stein operator as the optimal choice of $\hat{\phi}$:

Summing up, vanilla SVGD starts with the initial particle set $\{x_i^0\}_{i=1}^{n} \sim q_0(x)$, and updates them for iteration $l$ via:
...
\end{align}
Here, $\nabla_x \log p(x)$ is the gradient of the log probability density of the target distribution. This is also referred to as the \emph{score function} or simply the score. The use of the score is beneficial for multiple applications including Bayesian inference where computation of the typically intractable normalizing constant needs to be sidestepped \citep{JMLR:v6:hyvarinen05a}.
...
The Langevin-Stein operator can be interpreted as follows: the first term $\psi$ acts as a driving force or a confining force (CF) (as coined by \cite{ye_stein_2020}), that drives the particles towards regions towards the high probability regions of the posterior distribution. This primarily depends on the kernel-weighted score term --- for a single particle, the influence of the remaining particles on the total force diminishes with distance, with its closest neighbours determining the extent to which it is steered towards the high probability regions. 
The second term $\tau$ acts as a repulsive force (RF) that prevents mode collapse and drives particles apart at each SVGD iteration. If a stationary kernel supported in the Stein class of smooth densities is used, this depends only on the distance between pairs of particles and hyperparameters such as the lengthscale, which can be adapted iteration to iteration based on the previous set of particles. 
...
Common implementations of SVGD, including that of \cite{Liu2016} use AdaGrad [CITE] to drive the system to convergence in fewer iterations. AdaGrad adjusts the base step-size $\epsilon_l$ per particle based on the inverse of the particle gradient history. 
...
While the above advancements result in a very flexible framework that can capture correlation effects and non-Gaussian structure, the approach suffers from some key connected drawbacks, which we survey below:

\begin{enumerate}
    \item The curse of dimensionality: Much like kernel-based hypothesis testing, when the dimensionality of the problem far exceeds the number of particles, the method collapses to the MAP approximation, which may not provide adequate uncertainty quantification. The only solution would be to use a very large number of particles, which poses its own challenge in terms of score and kernel evaluation costs.
    \aj{connect to next point i.e. cost of actually bringing in more particles}


    \item Computing the score function. For Bayesian Inference, treating $p$ as the target posterior, the score can be further decomposed as $\nabla_x \log p(x | \CD) =  \nabla_x \log (\CD | x) + \nabla_x \log p_0(x)$. This amounts to computation of the likelihood for each particle based on repeated evaluations of an expensive high-fidelity model. The recommendation of \cite{Liu2016} is to approximate the score term in the confining force estimator with subsampled mini-batches of the data, with a view to reduce the number of score evaluations at a single point $x$. A formal justification of this procedure appears in \cite{gorham_stochastic_2020}, where the stochastic Stein discrepancy is formulated and shown to converge to continuous SVGD in 1-Wasserstein distance. 
...
The kernel weighting factor as well as the particle repulsion term also have scope for cost savings through identification of special covariance structure as the map is iteratively constructed. 
...
Instead of speed-ups to the SVGD procedure based on a single target density, we propose taking advantage of multi-fidelity (MF) techniques to drive particles at each iteration based on a combination of the target score function and inexpensive score evaluations from lower-fidelity models. MF methods have gained traction for scientific applications and can be naturally extended to estimation of quantities in particle transport with a well-established use case in Bayesian inverse problems. The integration of efficient kernel approximations is left for future work, with focus on the score function bottleneck for the rest of this paper. We provide some background on multi-fidelity estimation using approximate control variates (ACVs) in the following subsection, before reviewing existing literature that leverages multiple models. 
...
The work of \cite{alsup_multilevel_2022} proposes a hierarchical multi-level surrogate model. Given a sequence of measures $p^{(1)}, \cdots, p^{(L-1)}$ ordered by increasing costs that converge weakly to the target measure $p$ for $l \rightarrow \infty$, they demonstrate that the high computational costs needed to approximate the target directly can be reduced considerably by running SVGD at the highest level $L$ with a good initial distribution $q_0$ that is determined from SVGD iterations on the cheaper, lower levels. 
They demonstrate their methodology on Bayesian inverse problems, where such a sequence can be constructed from finite-difference or finite-element discretizations of the forward operator in a PDE. The method is easy to implement with multiple vanilla SVGD runs and requires little-to-no hyperparameter tuning. 
However, like most multi-level analysis, it is somewhat restrictive in its assumptions on the cost and model error relationships, and may not be able to accomodate arbitrary measure sequences derived from various data-driven models that may be available to the practitioner. 

Moreover, accurate estimation of the SVGD gradient given a finite interacting particle system is not tackled in the above method, which provides computational speedups through the limited number of iterations needed at the highest level rather than through the evaluation of fewer high-fidelity particles. 
The exact trade-off between these costs remains an open question and could motivate more powerful algorithms that exploit both avenues for cost savings i.e. methods that propagate particles under all fidelities simultaneously while adaptively optimizing the computational workload between different fidelities based on suitable measures of convergence to the target. Our work seeks to explore a simplified version of this.

Finally, the work of \cite{weissmann_mean-field_2024} explores a multi-level method that 


This relies on .... On the other hand, our method does not alter the estimators for the posterior moments, but uses multi-fidelity estimation of the SVGD gradient at every step. The empirical measure from the final particle ensemble obtained under this method is then used to compute posterior statistics.
