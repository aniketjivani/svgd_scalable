...
We wish to approximate $\PP$ with a set of particles $\{x_i\}_{i=1}^N$ whose empirical measure $\hat{\mu}_N(dx) = \sum_{i=1}^{n} (\delta (x - x_i) /N ) dx$ weakly converges to $\PP$ as $n \rightarrow \infty$, in the sense that $\EE_{\hat{\mu}_n}[h] \rightarrow \EE_{\PP}[h]$ as $N \rightarrow \infty$ for bounded, continuous test functions $h$.
...
\begin{enumerate}
    \item \emph{The curse of dimensionality}: When the dimensionality of the problem far exceeds the number of particles, the repulsive force which ensures adequate coverage of the posterior distribution becomes vanishingly small. As a result, the method collapses to the MAP approximation, which may not provide adequate uncertainty quantification. This motivates techniques that exploit any low-dimensional structure in the problem in conjunction with the propagation of a large number of particles. The latter however poses its own challenges in terms of score and kernel evaluation costs.
    \label{it: curse_dim}
...
Quantifying and exploiting the trade-off between these costs remains an open question. This motivates potential methods that can, for instance, propagate particles under all model fidelities simultaneously while distributing the computational workload optimally for variance reduction. 
...
For particle $x_i^{(l)}$, we denote all particles participating in its confining force update by $z^{(i, l)}$. We note that $z^{(i, l)}$ is typically made up of all particles in $\mathbf{x}^{(l)}$ in regular SVGD methods, but in stochastic approximations to SVGD can consist of a random subset of its neighbours.
...
\begin{align}
    x_i^{l + 1} &\leftarrow x_i^l + \epsilon_l \textcolor{orange}{\hat{\psi_0}} + \epsilon_l \hat{\tau_0}
    \label{eq: exp_partition}
\end{align}
where
\begin{align}
    \hat{\Psi}_0(x_i^{(l)}, z^{(i, l)}) = \frac{1}{N}\sum_{j=1}^N \psi(z_j^{(i, l)}, x)=\frac{1}{N}\sum_{j=1}^N k(z_j^{(i, l)}, x_i^{(l)}) \nabla_{z_j^{(i, l)}}\log p_0(z_j^{(i, l)})
...
where
\begin{align}
        \tilde{\Psi}(x_i^{(l)}, z^{(i,l)}, \alpha, \CA) = \hat{\Psi_0}(x_i^{(l)}, z^{(i, l)}) + \sum_{m=1}^M \alpha_m \left(\hat{\Psi}_m(x_i^{(l)}, z_m^{(i, l)\ast}) - \hat{\gamma}_m(x_i^{(l)}, z_m^{(i, l)})\right),
...
For each particle of the ensemble, the optimal values of the ACV hyperparameters $\alpha$ and $\CA$ depend on the computational costs and the model output covariance matrix $C$ for the vector of CF outputs $\psi(x_i^{(l)}, z^{(i, l)}) = [\psi(x_i^l, z_1^{i,l}), \cdots, \psi(x_i^l, z_M^{(i,l)})]$. Typically, we do not have access to the true output covariance, and instead must estimate it through pilot sampling. For ACV-SVGD, we estimate $C$ via an offline pilot sampling step, where an independent set of pilot particles is first propagated to the high-fidelity target density, and the CF is computed for the resulting particle trajectory under lower-fidelity targets.
...
We first demonstrate the method on two benchmark problems, each with 2 low-fidelity models: multivariate Gaussians $\{p_i(x)\}_{i=0}^{M-1}$, with $M=3$, and each $p_i$ parametrized by its respective mean and covariance $(\mu_i, \Sigma_i)$, and the classical Neal's funnel \citep{neal_slice_2003} in two dimensions. We also extend the Gaussian case to higher dimensions to evaluate the feasibility of ACV-SVGD in those settings.
...
multifidelity estimation based on approximate control variates to Stein Variational Gradient Descent (SVGD), a popular Bayesian inference method. This establishes an unbiased estimate of the gradient at each SVGD iteration, formed by propagating subsets of particles under low-fidelity target densities. We learn the control variate weights and a sample allocation strategy to distribute particles amongst various fidelities, which serve as hyperparameters to the ACV estimator. Combined with randomization of the designated high-fidelity subsets at each step, the method can propagate far more particles than its single-fidelity version under identical budgets. It results in very small gains in accuracy when estimating posterior statistics, and is fairly robust over different initialization distributions and replications.

Several promising directions exist for future work. First, adapting the multi-fidelity procedure for application to Bayesian neural networks would be of great interest, as it can further enhance the accuracy and cost savings from pSVGD \citep{peng2020} and ccSVGD\citep{padmanabha_csvgd_2025} through use of a greater number of particles and potentially surrogate models for the score. Second, the offline pilot sampling procedure can be refined further and incorporated into the ACV-based propagation through online pilot sampling techniques. For instance, the particle ensemble derived from the ACV-estimate at the previous iteration can be exploited to update the confining force covariances. Finally, the current method chooses to include all particles in the update step instead of leveraging stochastic approximations \citep{das2023}, while also being constrained to run at a fixed step size for the pilot and estimation stages. Tuning the step size instead to ensure better correlation, potentially informed by optimal transport couplings between fidelity levels could yield more gains from the multi-fidelity framework in budget-constrained settings. This also opens up the possibility to use learning-rate free procedures such as \cite{sharrock_coin_2023} that speed up convergence to the target.
