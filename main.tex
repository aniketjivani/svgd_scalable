...
Stein Variational Gradient Descent (SVGD) is a popular non-parametric variational inference technique for Bayesian inference. Typically, a finite set of particles is transported deterministically to characterize complex posterior distributions without restriction on the variational family. This only requires access to the score function of the target density and a standard kernel matrix to determine confining and repulsive forces acting on the interacting particle system.
Despite the particle efficiency compared to standard Monte Carlo estimation, usage of SVGD is bottlenecked by the cost of score evaluation for computationally expensive high-fidelity forward models and high-dimensional surrogates, which are ubiquitous in scientific applications.
