...
However, SVGD struggles to accomodate many-query evaluations of the likelihood function when a computationally expensive high-fidelity forward model is involved, which is very common in scientific applications.
...
The key difference compared to the surrogate SVGD estimator we discussed in the previous section would stem from the unbiasedness of the ACV estimator.


Following the notation introduced in \autoref{ss: acv}, we can in fact propose multiple ACV estimators, depending on how we split the terms in the vanilla SVGD gradient:

\begin{enumerate}
\item Decomposing the kernel-weighted scores and the repulsive force terms:
\begin{align}
        \tilde{\phi}(x, \alpha, \CA) = \tilde{\phi_0}(x_0) + \sum_{m=1}^M \alpha_m \left(\hat{\phi}_m(x_m^{\ast}) - \hat{\mu}_m(x_m)\right)
\end{align}

followed by:
\begin{align}
    x_i^{l + 1} \leftarrow x_i^l + \epsilon_l\tilde{\phi}(x_i^l, \alpha, \CA)
...
\begin{align}
        \tilde{\psi}(x, \alpha, \CA) = \tilde{\psi_0}(x_0) + \sum_{m=1}^M \alpha_m \left(\hat{\psi}_m(x_m^{\ast}) - \hat{\gamma}_m(x_m)\right)
\end{align}
followed by:
\begin{align}
    x_i^{l + 1} \leftarrow x_i^l + \epsilon_l\tilde{\psi}(x_i^l, \alpha, \CA) + \epsilon_l\left[\frac{1}{n}\sum_{j=1}^{n} \nabla_{x_j^l} k(x_j^l, x)\right]
    \label{eq: mf_svgd_proposal_2}
\end{align}
where $\tilde{\psi}_{(\cdot)}$ is shorthand for $\left[\frac{1}{n_i} \sum_{j=1}^{n_i}k(x_j^l, x) \nabla_{x_j^l} \log p_i (x_j^l)\right]$ and $n_i$ denotes the fraction of particles out of $n$ particles used in the score computation for model $i$.
\end{enumerate}

Other choices that we do not focus on here may construct separate MF estimators for each term of the gradient, or alternately propose a nested estimator, where the partitioning is performed for the kernel computations, followed by an additional allocation step to models of different fidelities. We leave exploration of the implications resulting from these choices to future work and discuss the above choices in greater detail as a first step towards a multi-fidelity algorithm.
