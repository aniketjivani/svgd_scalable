...
\begin{enumerate}
    ...
    \textcolor{red}{Instead, can we just use a stand-alone approximation like KISS, which doesn't need to be combined with GP fitting?}

    \item What are the precise connections between SVGD and flow-based models like CNFs?


    \textcolor{blue}{The simplest thing we can actually try, funnily enough, is just replacing $p(.)$ by some MLP and / or a richer kernel representation for $k$ e.g. spectral delta kernel - wondering if the latter has any nicer properties - the point being that we can get rid of the variational part and consider a more general case.}
\end{enumerate}
...
(doing what's already been done before and isn't very remarkable once I think about it a little more) Connecting trace and Stein operator from Liu's paper...

\section{General Literature on Stein}
