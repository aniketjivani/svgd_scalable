...
\aj{add to literature: Other Bayesian Inference techniques e.g. SGLD (\url{https://www.stats.ox.ac.uk/~teh/research/compstats/WelTeh2011a.pdf}), MCMC related: Support points (VR Joseph) and Stein Points, VP and GB- SVGD (for convergence), thinning for MCMC \url{https://arxiv.org/pdf/2005.03952}, actual multi-level approximation - \url{https://arxiv.org/pdf/2402.01320}. Other SVGD techniques from Cosmin e.g. condensed SVGD?}. Also annealed SVGD (KAUST) and MP-SVGD (Zhuo et al. for the variance underestimation etc. arguments) - show performance in slightly higher number of dimensions. Finally, highlight role in diffusion models - Stein Score distillation - \url{https://arxiv.org/abs/2401.00604}. Also CDF estimation (Boris Kramer) - \url{https://arxiv.org/abs/2303.06422}
...
Let $\pi_0$ denote the prior distribution on $\mathbf{x}$ and consider zero-mean Gaussian noise with covariance $\Gamma$. Using identical prior specification for each process in Eq.~\eqref{eq: bip_fm}, the posterior distribution is given by Bayes' rule:

\begin{equation}
    p(\mathbf{x}) = \frac{1}{Z} \exp \left(-\frac{1}{2}||y - f_i(\mathbf{x})||_{\Gamma^{-1}}^2\right)\pi_0(\mathbf{x})
    \label{eq: post}
\end{equation}

where $Z$ is the normalizing constant, typically intractable in most problems of interest. When applying SVGD, this drops out because of the use of the score function:
\begin{align}
    \nabla_{x} \log p(\mathbf{x}) = \nabla_x (L_i  + \log(\pi_0(\mathbf{x})),
    \label{eqn: bayes_rule_score}
\end{align}

We consider two problems of interest both involving a steady state elliptic PDE:
\begin{enumerate}
    \item Steady-state diffusion based on the setup in \cite{alsup_multilevel_2022}, with $d=2$, and each model considering finite-difference discretizations at 1024, 64 and 16 grid points respectively.
