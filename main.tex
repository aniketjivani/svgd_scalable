...
\ifpdf
\hypersetup{
  pdftitle={Towards Multi-fidelity Stein Variational Gradient Descent with Application to Bayesian inverse problems},
...
We demonstrate the value of the method in numerical benchmark problems, including Bayesian inference depending on a PDE forward model through the likelihood.
...
\begin{enumerate}
    ...
    \item We investigate the effect of pilot sampling that is key to establishing covariance information for learning ACV hyperparameters. 
    
    \item We validate the method on benchmark target densities and a Bayesian inverse problem.
\end{enumerate}

These contributions provide a novel way to leverage control-variate techniques in settings of approximate inference, targeting intermediate update steps rather than the statistics of the posterior distribution directly. 
The remainder of this paper is organized as follows. Section 2 reviews SVGD literature, some background on ACV estimators and discusses related work. Section 3 introduces the new multi-fidelity SVGD update and the overall algorithm with the pilot sampling procedure. Section 4 demonstrates the method on several inference problems. Section 5 concludes.
...
% Also annealed SVGD (KAUST) and MP-SVGD (Zhuo et al. for the variance underestimation etc. arguments) - show performance in slightly higher number of dimensions. 
% Finally, highlight role in diffusion models - Stein Score distillation - \url{https://arxiv.org/abs/2401.00604}. Also CDF estimation (Boris Kramer) - \url{https://arxiv.org/abs/2303.06422}. Non-asymptotic analysis: \citep{Korba2020}
...
Let $x$ denote an unobservable quantity or parameter, and $D$ denote data. In order to estimate $x$, we define and decompose a joint probability distribution on $x$ and $D$ as follows:
\begin{align}
    \pi(x, D) = \underbrace{\pi(x)}_{\text{prior}} \underbrace{\pi(D\mid x)}_{\text{likelihood}}
\end{align}

The object of interest is the ``posterior density'' $p(x \mid D)$, the updated uncertainty for the unknown parameters conditioned on newly acquired observations. It is obtained through the re-arrangement of the various conditional probabilities via Bayes' rule:

\begin{align}
    \pi(x \mid D) = \frac{\pi(D\mid x) \, \pi(x)}{\pi(x)},
    \label{eqn: bayes_rule_v1}
\end{align}

where $\i(D)$ is the evidence or marginal likelihood, the usually intractable normalizing constant.

Much of modern statistical inference with complex models relies on Markov Chain Monte Carlo (MCMC) methods \citep{brooks_handbook_2011}, where a sequence of random samples is generated through the construction of a Markov chain whose equilibrium distribution is the target posterior distribution. It has the key benefit of asymptotic convergence guarantees while sampling from arbitrary distribution families. However, these methods remain computationally intensive when dealing with high-dimensional parameter spaces, despite impressive algorithmic advancements (for e.g., \cite{neal_mcmc_2012,JMLR:v15:hoffman14a}) that make use of gradient information to guide new proposals.

On the other hand, Variational Inference (VI)\citep{jordan_introduction_1999,blei_variational_2017} reformulates inference as an optimization problem, where the posterior is approximated by a parametric family of distributions that is closest to the true posterior through a suitable measure. This is usually the reverse KL divergence or other measures from the family of $f$-divergence between probabiliy distributions. 
...
% \aj{Stein Discrepancy definition - use \cite{gorham_measuring_2017} and Grathwohl 2020 (Learning Stein Discrepancy for Evaluating EBMs) to provide original definition of and justification of kernelized version of SD}

Stein Variational Gradient Descent (SVGD), introduced in \cite{Liu2016}, is a powerful method that bridges the gap between VI and MCMC, through the evolution of particles to characterize complex posterior distributions. We provide a brief overview of the method here, and refer readers to \cite{Liu2016} for full details of the derivation.

% \aj{Need to paraphrase a little from Liu 2017 and remove notation that is redundant}

Consider a probability space $(\Omega, \CF, \PP)$ where $\Omega$ is the sample space, $\CF$ is a $\sigma$-algebra, and $\PP$ is a probability measure with a positive, weakly differentiable density $p(x)$. 
% \aj{$\int_{\Omega} p \partial_i\phi dx = - \int_{\Omega} g_i \phi dx$ for compactly supported $\phi$ then $g_i$ is weak ith derivative of $p$}. 
We wish to approximate $\PP$ with a set of $d$-dimensional particles $\{x_i\}_{i=1}^N$, $x_i \in \RR^d$ whose empirical measure $\hat{\mu}_N(dx) = \sum_{i=1}^{n} (\delta (x - x_i) /N ) dx$ weakly converges to $\PP$ as $N \rightarrow \infty$, in the sense that $\EE_{\hat{\mu}_n}[h] \rightarrow \EE_{\PP}[h]$ as $N \rightarrow \infty$ for bounded, continuous test functions $h$.

We initialize particles from an arbitrary starting distribution $q_0$ and transport them over multiple iterations via the map $T$:
\begin{align}
...
The choice of an appropriate set of transforms in this method is connected to Stein's identity \citep{stein_bound_1972}:
\begin{align}
    \EE_{x\sim p}[\CS_p \phi(x)] = 0, \label{eq: steins_id}
\end{align}
where $\phi$ is a smooth function and $\CS_p \phi(x) = \phi(x) \nabla_x \log p(x)^{\top} + \nabla_x \phi(x)$ is the so-called Langevin-Stein operator. $\nabla_x \log p(x)$ is the gradient of the log probability density of the target distribution, also referred to as the \emph{score function} or simply the score \citep{JMLR:v6:hyvarinen05a}.
% Under a different smooth density $q(x)$ proposed to approximate the target, the maximum violation of Stein's identity in a proper function set $\CG$ defines a discrepancy measure known as the Stein discrepancy:
% \begin{align}
%     \mathbb{S(q, p)} = \max_{\phi \in \CG} \left[\EE_{x\sim q} \operatorname{tr}\left(\CA_p\phi(x)\right)^2\right]
% \end{align}

The key result of \cite{Liu2016} chooses the optimal transform at each step to maximally decrease the KL divergence of the particle distribution with the target distribution $p$, described by the following functional optimization problem:
\begin{align}
    \hat{\phi}^{\ast} = \arg \max_{\phi_\in \CH} \left\{-\frac{d}{d\epsilon}\operatorname{KL}(q_{[T]}|| \PP)|_{\epsilon=0} \quad s.t. ||\phi||_{\CH} \leq 1\right\}\label{eq: kl_opt}
\end{align}

Here, $q_{[T]}(z)$ denotes the density under the perturbation mapping $z=T(x)$
% $T_{\sharp} \mu$ 
% is the pushforward measure of $\mu$ through map $T$, 
and $\CH$ is a reproducing kernel Hilbert Space (RKHS), equipped with a positive definite kernel $k(x, x'): \CX \times \CX \to \mathbb{R}$. The solution to Equation~\eqref{eq: kl_opt} leads to the vanilla SVGD algorithm. Starting from the initial particle set $\{x_i^0\}_{i=1}^{n} \sim q_0(x)$, we update them for iteration $l$ via:
...
\begin{align}
    ...
    \text{ where }\hat{\phi}^{\ast}(
    \cdot
    ) &= \EE_{q^l}[k(x^l, \cdot)\nabla_x \log p(\cdot) + \nabla_x k(x, \cdot)] \nonumber \\
    &= \frac{1}{N}\left[\textcolor{orange}{\underbrace{\sum_{j=1}^N k(x_j^l, x) \nabla_{x_j^l}\log p(x_j^l)}_{\text{Confining Force (CF) }, \psi}} + \textcolor{violet}{\underbrace{\sum_{j=1}^{N}\nabla_{x_j^l}k(x_j^l, x)}_{\text{Repulsive Force (RF)},\tau}}\right].
    \label{eq: vanilla_svgd}
\end{align}

The action of each SVGD step can be interpreted as follows: the first term $\psi$ acts as a driving force or a confining force (CF) (as coined by \cite{ye_stein_2020}), that drives the particles towards regions towards the high probability regions of the posterior distribution. This is expressed through the kernel-smoothed score term --- for a single particle, the influence of the remaining particles on the total force diminishes with distance, with its closest neighbours determining the extent to which it is steered towards the high probability regions. 
The second term $\tau$ acts as a repulsive force (RF) that prevents mode collapse and drives particles apart at each SVGD iteration. If a stationary kernel that is supported in the Stein class of smooth densities (for example, standard RBF and IMQ kernels) is used, this depends only on the distance between pairs of particles and hyperparameters such as the lengthscale, which can be adapted iteration to iteration based on the previous set of particles. 
As the SVGD update progresses, these two competing forces eventually balance each other, so the approximate density $q$ weakly converges to the target $p$. Thus, SVGD evolves a co-ordinated particle ensemble. 

When the target density is a posterior distribution (as in Equation~\eqref{eqn: bayes_rule_v1}), the $n$-particle representation of the posterior takes the form:

\begin{align}
    x_i^{l + 1} &\leftarrow x_i^l + \frac{\epsilon_l}{n} \left[\sum_{j=1}^n k(x_j^l, x) \left(\nabla_{x_j^l}\log p(D|x_j^l) + \nabla_{x_j^l} \log p(x_j^l)\right) + \nabla_{x_j^{(l)}} k(x_j^{(l)}, x)\right]
    \label{eq:posterior_vanilla_svgd},
\end{align}
where the score function is independent of the normalization of $p(x|D)$ and thus sidesteps determination of $p(D)$. In the extreme case of a single particle where $\nabla_xk(x, x)=0$, the method reduces to the typical gradient ascent procedure for estimating the Maximum-a-posteriori (MAP) of the distribution.
% \citep{JMLR:v6:hyvarinen05a}. 
...
Common implementations of SVGD, including that of \cite{Liu2016} use AdaGrad\citep{duchi11a} to drive the system to convergence in fewer iterations. AdaGrad adjusts the base step-size $\epsilon_l$ per particle based on the inverse of the particle gradient history. 

Since its initial development, a number of follow-up works have sought to enhance SVGD both theoretically and algorithmically, to  increase its utility in real-world Bayesian inference tasks. \cite{detommaso_stein_2018} accelerate the SVGD algorithm by including second-order information, resulting in a Stein Variational Newton (SVN) method. The work of \cite{wang_stein_2019} generalizes the SVGD further to make use of flexible matrix-valued kernels in a preconditioned gradient descent procedure. A special case of this procedure recovers the SVN method. Overall, these works help accelerate the exploration of the target density landscape and provide greater flexibility in the choice of RKHS kernels.

Other works seek faster convergence to the target based on robust versions of SVGD that are not as sensitive to the choice of hyperparameters such as the initial distribution $q_0$ and the learning rate schedule $\{\epsilon_l\}$.
% , targeting convergence issues arising from both the choice of initialization and the selection of a suitable learning rate or the step size sequence. 
For instance, when approximating multimodal target densities, particles may end up in modes closest to their initializations and are unable to explore all the posterior modes. \cite{dangelo2021annealed} propose an annealing schedule inspired by similar work in MCMC methods that rescales the target density by a temperature parameter and promotes exploration during the initial stages to mitigate effects of poor initialization. 
Ensuring convergence within a reasonable number of iterations also requires careful selection of the learning rate sequence --- however the learning rate is a function of the unknown target measure, thus determining an optimal sequence in practice requires tuning through methods like grid search. To improve the flexibility of the algorithm, a method  known as \texttt{CoinSVGD} \citep{sharrock_coin_2023} establishes a learning-rate free method rooted in betting strategies for stochastic optimization, that substantially improves the rate of convergence without requiring the user to set an explicit schedule. 

Concurrently, our theoretical understanding of SVGD has improved substantially. \cite{liu_stein_2017} develops the first theoretical analysis of SVGD, characterizing its asymptotic behaviour by the nonlinear Vlasov equation and drawing connections to transport methods based on gradient flows of other discrepancy measures. Others works develop theory on related applications of Stein's identity, and impact practical choices for SVGD in the process, such as the metrics used to track SVGD progress. For instance, the weak convergence of the Kernelized Stein Discrepancy (KSD), which measures violation of Stein's identity under approximations to $p$, is examined in \cite{gorham_measuring_2017}, and suggests that commonly used KSD formulations fail to detect non-convergence for violating sample sequences, even for simple Gaussian targets in small to modest dimensions ($d \geq 3$). They motivate the use of kernels with slowly decaying tails, such as the Inverse Multi-Quadric (IMQ) kernel for KSD to provably determine convergence for a large class of target distributions.
\cite{shi_mackey_2023} provided the first finite-particle convergence rate under mild assumptions, where the kernelized Stein discrepancy is driven to zero at a rate of $(1/\sqrt{\log \log n})$ for $n$ particles. Follow-up works, such as \cite{das2023} provide much better guarantees that better explain the practical performance of SVGD through random-batch approximation algorithms, and establish polynomial dimension dependence.
...
% Finally, we emphasize important adaptations of SVGD to settings such as training of Bayesian Neural Networks, marked by high-dimensionality of the parameter space and a complex posterior landscape. 
...
While the above advancements result in a very flexible framework that can capture correlation effects and non-Gaussian structure, the approach suffers from some key closely connected drawbacks, which we survey below:
\begin{enumerate}
    \item \emph{The curse of dimensionality}: When the dimensionality of the problem $d$ far exceeds the number of particles $N$, the repulsive force which ensures adequate coverage of the posterior distribution becomes vanishingly small. As a result, the method collapses to the MAP approximation, which may not provide adequate uncertainty quantification. This motivates techniques that exploit any low-dimensional structure in the problem in conjunction with the propagation of a large number of particles. The latter however poses its own challenges in terms of score and kernel evaluation costs.
    \label{it: curse_dim}
    % \aj{connect to next point i.e. cost of actually bringing in more particles}


    \item \emph{Score computation}: For Bayesian Inference, treating $p$ as the target posterior, the score can be further decomposed as $\nabla_x \log p(x | \CD) =  \nabla_x \log (\CD | x) + \nabla_x \log p_0(x)$. This amounts to computation of the likelihood for each particle based on repeated evaluations of an expensive high-fidelity model. 
    \label{it: score_comp}
    
    \item \emph{Kernel computation}: The $O(n^2)$ cost associated with computing elements $k(x_i, x_j)$ of the dense kernel matrix. For arbitrary kernel choices where simple heuristics such as the median trick for the lengthscale do not apply, the selection of hyperparameters becomes an additional challenge to performant SVGD updates.\label{it: kernel_cost}
\end{enumerate}


% While these can bring about significant computational savings, they do not consider reduction in cost of an individual forward model evaluation that drives the score update.

To address (\ref{it: score_comp}), the original recommendation of \cite{Liu2016} is to approximate the score term in the confining force estimator with subsampled mini-batches of the data, with a view to reduce the number of score evaluations at a single point $x$. A formal justification of this procedure appears in \cite{gorham_stochastic_2020}, where the stochastic Stein discrepancy is formulated and shown to converge to continuous SVGD in 1-Wasserstein distance. For (\ref{it: curse_dim}), the frameworks proposed in \cite{peng2020} and \cite{anantha_padmanabha_improving_2024,padmanabha_csvgd_2025} exploit active-subspace based dimensionality reduction and sparsification techniques that enable running SVGD on a smaller and more interpretable parameter space, improving its accuracy and scalability in the process. This enhances the ability of SVGD to characterize complex and overparametrized neural network posteriors.

The particle repulsion term (\ref{it: kernel_cost}) also has scope for cost savings through identification of special covariance structure during SVGD propagation. 
...
Since particles assume memberships of separate clusters, this introduces structure and sparsity in the covariance matrix, which may be exploited to bring down the total kernel evaluation cost. For instance, a message-passing version of SVGD \citep{zhuo18a} is tailored to continuous graphical models through localized kernel approximations. Estimation of sparse covariance structures \citep{fop_model-based_2018} is of independent interest in the statistical literature and in studies of  graphical models.
...
Instead of speed-ups to the SVGD procedure that drives particles to cover a single target density, we propose taking advantage of multi-fidelity (MF) techniques to drive a mixture of particles at each iteration under target densities constructed from the high-fidelity and multiple low-fidelity models. MF methods have gained considerable traction for scientific applications and have the potential to improve estimation of quantities in particle-based VI with a pertinent use case in Bayesian inverse problems. The integration of efficient kernel approximations is left for future work, with focus on the score function bottleneck for the rest of this paper. We provide some background on multi-fidelity estimation using approximate control variates (ACVs) in the following subsection, before reviewing existing literature that leverages multiple models. 
...
To reduce the estimator variance, multi-fidelity methods leverage an ensemble of low-fidelity (LF) models whose outputs are correlated with the high-fidelity (HF) model of interest but with diminished costs. 
...
Typically, selection of $f_1$ (generalized to selection of $\{f_i\}, i=1, \cdots, M-1$) and identifying a rich set of candidate CVs is a non-trivial procedure barring the exceptions where CVs can be identified from domain knowledge. 
...
In the context of multi-fidelity methods however, we typically already have access to a hierarchy of models, where each available low-fidelity (LF) model can be leveraged as a control variate, but their respective expectations must be computed from samples too. The approximate control variate (ACV) technique \cite{gorodetsky_generalized_2020, bomarito_optimization_2022} provides a rigorous formulation for deriving reduced-variance estimators in this setting.
This method introduces $(M - 1)$ auxiliary random variables $Q_{m}$ that are outputs of the corresponding low-fidelity models $Q_{m} =f_{m}(Z)$, then combines the corresponding MC estimates of each low-fidelity model, $\hat{Q}_{m}$, into a single estimator for the expected value of $Q_0$, the high-fidelity (HF) model . The ACV estimator, which we denote $\tilde{Q}$, can be written as:
\begin{align}
        \Tilde{Q}(z,\alpha,\CA) &:= 
        \hat{Q}_{0}(z_{0})+\sum^{M-1}_{m=1}\alpha_{m}\left( \hat{Q}_{m}(z^{\ast}_{m})-\hat{\mu}_{m}(z_{m}) \right) \nonumber\\ 
        &= \hat{Q}_{0}(z_{0})+\sum^{M-1}_{m=1}\alpha_{m}\left( \hat{Q}_{m}(z^{\ast}_{m})-\hat{Q}_{m}(z_{m}) \right),     \label{eqn:ACV-Formula}
\end{align}
...
\right]$, as well as the model costs vector $w$. Since $\Sigma$ is typically unknown \emph{a priori}, it is commonly estimated using $N_{\text{pilot}}$ independent pilot evaluations via the unbiased sample covariance.
...
In this section, we highlight two previous approaches that leverage estimation with multiple models to drive the SVGD updates and achieve significant cost and / or variance reduction in the process. 

The work of \cite{alsup_multilevel_2022} does not combine outputs from lower-fidelity SVGD runs directly but instead proposes a hierarchical multi-level surrogate model. Given a sequence of measures $p^{(1)}, \cdots, p^{(L-1)}$ ordered by increasing costs that converge weakly to the target measure $p$ for $l \rightarrow \infty$, they demonstrate that the high computational costs needed to approximate the target directly can be reduced considerably by warm-starting SVGD at the highest level $L$ with a good initial distribution $q_0$ that is determined from SVGD iterations on the cheaper, lower levels. The gradient norm is used as a stopping criterion at each level. Once the highest level converges to the target, the computational costs arising from all levels are compared to the cost of running vanilla SVGD at the highest level from the outset.
They demonstrate the speedups from their methodology on Bayesian inverse problems, where such a measure sequence can be constructed from successively finer finite-difference or finite-element discretizations of the forward operator in a partial differential equation (PDE). The method is easy to implement with multiple vanilla SVGD runs and requires little-to-no hyperparameter tuning. 
However, like most multi-level analyses, it is somewhat restrictive in its assumptions on the cost and model error relationships, and may not be able to accomodate measure sequences derived from arbitrary data-driven models available to the practitioner. There is no explicit comparison between subsets of lower-fidelity levels either, with all available levels being used irrespective of their potential for cost savings. 

Moreover, it does not address accurate estimation of the SVGD gradient given a finite interacting particle system. The computational speedup is achieved through the limited number of iterations needed at the highest level rather than through the evaluation of fewer high-fidelity particles. 
Quantifying and exploiting the trade-off between these costs remains an open question. This motivates potentially better methods that can, for instance, propagate particles under all model fidelities simultaneously while distributing the computational workload optimally for variance reduction. 
% Our work seeks to explore a simplified version of this.

 Another related work \citep{weissmann_mean-field_2024} explores variance reduction through multi-level approximation of the expectation of an output QoI $Q_0$ evaluated at samples from the posterior distribution. This translates to a multi-level estimator for $q_{[T]}$ constructed through a telescoping sum of the empirical measure obtained independently at various levels. While appealing from the standpoint of unbiased estimation of posterior statistics and the ability to use varying particle ensemble sizes at each level, their approach too is limited by the available choices for low-fidelity model construction. Further, they use fixed (unitary) control-variate weights that may lead to sub-optimal variance reduction, and variance increase in the worst case when correlations drop below a certain threshold, as noted in \cite{sukys_optimal_2017,gorodetsky_generalized_2020}.

On the other hand, our method does not target the estimators of output functionals directly, where multi-fidelity estimation would be applied only once following convergence of the independent SVGD chains from different fidelities. Instead, we perform multi-fidelity estimation and variance reduction for the HF SVGD \emph{gradient} to propagate a single collection of particles at every iteration. The final particle ensemble obtained via this technique is then used to characterize the posterior, compute summary statistics, posterior predictive density and so on. While the previous work did not necessitate a pilot sampling step, it is a prerequisite to the ACV estimation proposed here. Therefore,  we also empirically investigate the effect of varying pilot budget allocation on the results, compared to the single-fidelity case that uses no pilot samples.
...
\section{Methodology}
\subsection{A Multi-fidelity Estimator for SVGD}
...
We let $\mathbf{x}^{(l)} = [x_1^{(l)}, \cdots , x_N^{l}]$ denote positions of $N$ particles used in the SVGD approximation at iteration $l$. 
% For particle $x_i^{(l)}$, we denote all neighbouring particles by $z^{(i,l)} \triangleq \mathbf{x}^{(l)} \setminus x_i^{(l)}$. 
For particle $x_i^{(l)}$, we denote all particles participating in its confining force update by $z^{(i, l)}$. We note that $z^{(i, l)}$ is typically made up of all particles in $\mathbf{x}^{(l)}$ in regular SVGD methods, but for stochastic approximations to SVGD can consist of a random subset of its neighbours.
First, by linearity of expectation, we partition the overall gradient in Equation~\eqref{eq: vanilla_svgd} for a high-fidelity target density $p_0$:
\begin{align}
    x_i^{l + 1} &\leftarrow x_i^l + \epsilon_l \textcolor{orange}{\hat{\Psi_0}} + \epsilon_l \hat{\Gamma_0}
    \label{eq: exp_partition}
\end{align}
where
\begin{align}
    \hat{\Psi}_0(x_i^{(l)}, z^{(i, l)}) = \frac{1}{N}\sum_{j=1}^N \psi_0(z_j^{(i, l)}, x)=\frac{1}{N}\sum_{j=1}^N k(z_j^{(i, l)}, x_i^{(l)}) \nabla_{z_j^{(i, l)}}\log p_0(z_j^{(i, l)})
    \label{eq: cf_expression_hifi}
\end{align}
and
\begin{align}
    \hat{\Gamma}_0(x_i^{(l)}, z^{(i, l)}) = \frac{1}{N}\sum_{j=1}^N \nabla_{z_j^{(i, l)}} k(z_j^{(i, l)}, x_i^{(l)})
    \label{eq: rf_expression_hifi}
\end{align}
    
% An update for particle $x_i^{l}$ involves evaluation of the CF and RF terms at $x_i^l$ as well as its neighbours $z_i^l \triangleq \mathbf{x}^{(l)} \setminus x_i^l$. 
The repulsive force computation, which depends only on the positions of particles at a particular iteration, is not affected by the choice of score model and therefore does not admit a control-variate representation based on the score function. A multi-fidelity SVGD update thus replaces only the confining force by its ACV representation while retaining the single-fidelity estimator for the RF term. Consistent with the notation of Equation~\eqref{eqn:ACV-Formula}, we propose the modified gradient descent step:
\begin{align}
    x_i^{l + 1} &\leftarrow x_i^l + \epsilon_l \textcolor{orange}{\tilde{\Psi}} + \epsilon_l \hat{\Gamma_0}
    \label{eq: exp_partition_p_cf_acv}
\end{align}
where for each dimension $d$
\begin{align}
        \tilde{\Psi}^{(d)}(x_i^{(l)}, z^{(i,l)}, \alpha, \CA) = \hat{\Psi_0}^{(d)}(x_i^{(l)}, z_0^{(i, l)}) + \sum_{m=1}^{M-1} \alpha_m \left(\hat{\Psi}_m(x_i^{(l)}, z_m^{(i, l)\ast}) - \hat{\gamma}_m(x_i^{(l)}, z_m^{(i, l)})\right),
        \label{eq: cf_update_acv_formula}
\end{align}
where $\hat{\gamma}_m(x_i^{(l)}, z_m^{(i, l)})$ is the Monte-Carlo estimate for confining force of the $m$th score model, $\alpha_1, \cdots, \alpha_{M-1}$ are the control variate weights, and $z^{(i, l)}$ particles are partitioned into $z_0^{(i, l)},z_m^{(i, l)},z_m^{(i, l)\ast}$ according to sample partitioning strategy $\CA$.
% followed by:
% \begin{align}
...
% Other representations may also focus on reducing the computational cost of computing pairwise interactions apart from the score computation. We leave investigation of these choices to future work and discuss the ACV-based formulation in greater detail as a first step towards a flexible multi-fidelity version of SVGD.

Next, we discuss the selection of hyperparameters i.e. the control variate weights $\alpha$ and the sample partitioning $\CA$ for Equation~\eqref{eq: cf_update_acv_formula}.

\subsection{Selection of ACV-SVGD Hyperparameters}

For each particle of the ensemble, the optimal values of the ACV hyperparameters $\alpha$ and $\CA$ depend on the computational costs and the model output covariance matrix $\Sigma$.
% for the vector of CF outputs $\psi(x_i^{(l)}, z^{(i, l)}) = [\psi(x_i^l, z_1^{i,l}), \cdots, \psi(x_i^l, z_M^{(i,l)})]$. 
Typically, we do not have access to the true output covariance. It is commonly estimated through a pilot sampling step. If $B_P$ is the pilot sampling budget out of total computational budget $B$ corresponding to $N_{\text{pilot}}$ particles per iteration, the unbiased sample covariance formula can be used for individual particle $x_i^{(l)}$:
\begin{align}
    \hat{\Sigma}(x_i^{(l)}) = \frac{1}{N_{\text{pilot}} - 1}\sum_{j=1}^{N_{\text{pilot}}}(\psi^{(j)} - \bar{\psi})(\psi^{(j)} - \bar{\psi})^{\top},
\end{align}\label{eq: sample_cov_svgd}
where $\psi^{(j)}(x_i^{(l)}) = [\psi_0(z_j^{(i, l)}, x_i^{(l)}), \cdots, \psi_M(z_j^{(i, l)}, x_i^{(l)})]$ \textcolor{red}{fill in remaining notation}. Thus, every iteration $l$ requires optimization of hyperparameters for multiple outputs, each constituting a confining force update. 
% Approaches such as \cite{dixon_covariance_2024} provide specialized estimators that exploit the correlation between outputs for significant variance reduction. 
We follow the strategy outlined in \cite{Quaglino2019} that chooses a common sample allocation for all particles per dimension $\CA^d$ based on the scalarized objective:
\begin{align}
    \min_{\mathcal{A}^d\in\mathbb{A}} &\quad \sum_{j=1}^{N_p}\Var[\tilde{\Psi}_j^{(d)}(Z;\alpha^{\ast},\mathcal{A}^d, x_j^{(l)})] \label{eqn:mxmcpy_scalarized}\\
    \text{subject to} &\quad \mathcal{W}(w,\mathcal{A}^d)\leq B - B_P,\label{eqn:mxmcpy_constraint_scalarized}
\end{align}
where $w$ is the normalized cost vector ($w_0=1$) representing update cost of a single particle per iteration under each fidelity. While this is by far the most scalable approach as $(B-B_P)$ grows, there is also potential to exploit the correlations between gradient components for even greater variance reduction as done in the multi-output estimator formulation of \citep{dixon_covariance_2024}.

Once $\CA$ is optimized, the optimal control variate weights can be customized per particle (and potentially per dimension) via:
\begin{align}\label{eqn:alpha-star-acv_custom}
    \alpha^{d\ast}(\CA, x_i^{(l)})=-\Cov[\Delta_i^{(l)}, \Delta_i^{(l)}]^{-1}\Cov[\Delta_i^{(l)}, \hat{\Psi}_{0}^d(x_i^{(l)})]
\end{align}

The offline pilot sampling step, beginning with computation of Equation~\eqref{eq: sample_cov_svgd} requires access to a sequence of particle positions $X_p=[\mathbf{x}^{(0)}, \cdots, x^{(L-1)}]$. While these can be initialized arbitrarily, a practical choice for this is to first run SVGD for the high-fidelity target $p_0$ with $N_{\text{pilot}}$ particles per iteration, and store $\psi_0(z_j^{(i, l)}, \cdot)$ for each $x_i^{(l)}$. 
Next, we compute $\psi_m(z_j^{(i,l)}, \cdot)$ i.e. individual contributions to the confining force under low-fidelity target densities for the same sequence, and determine $\CA^d$. Assembling the CV weights through Equation~\eqref{eqn:alpha-star-acv_custom} also requires knowledge of CF covariance for particles in the ACV-SVGD loop. This step is addressed in the following subsection.



We summarize the pilot sampling procedure in algorithm 
...
% We use the simplest variation of ACV for our ACV-SVGD based framework that computes multi-fidelity estimates for a single output / scalar QoI. This implies that for a $d$-dimensional target posterior, the sample allocations for each update directions are computed separately based on optimal variance reduction in the respective components of the gradient / velocity field. 

\subsection{Inferring covariances through score emulation}

\begin{enumerate}
    \item Regular method - $\gamma$-GPs

    \item Computational cost of $\gamma$-GPs

    \item Score emulation:
    Using reference scores from all iterations obtained by running pilot SVGD, we fit a variational Gaussian Process with a small number of inducing points to emulate the score.

    Once a fresh set of particles is initialized to run ACV-SVGD, we use the fitted 
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{figures/score_surrogate.png}
    \caption{Example of score function emulation using a variational Gaussian process. (Left) Score surrogate predictions for $s(x_1, t)$ (Middle) True scores (Right) Predictive Uncertainty. We note that even at modest pilot budgets, the scores can be captured accurately}
...
\begin{algorithm}[H]
    \caption{ACV-SVGD}\label{alg:acv_svgd}
    \begin{algorithmic}[1] 
    \State \textbf{Input:} Step size $\epsilon$, target densities at different fidelity $p_i(x)$ for $i=0,\dots,M-1$, kernel $k(x,x')$, kernel lengthscale $h$, initial particles $\{x_j^{(0)}\}_{j=1}^{B_{\text{pilot}}}$, $B_{\text{pilot}}$, $B_{\text{ACV}}$.
    % \State \textbf{Options: } \texttt{ADAPT_LENGTHSCALE}, \texttt{USE_VANILLA_SVGD_GRADIENT}
    \State \textbf{Output: } Particles approximating the posterior after $L$ iteration: $\{x_j^{(L)}\}_{j=1}^{B_{\text{ACV}}}$
    \end{algorithmic}
\end{algorithm}
...
\subsection{Two-Dimensional Target Densities}

We first demonstrate the method on two benchmark problems, each with 2 low-fidelity models ($M=3$):
\begin{enumerate}
    \item Multivariate Gaussians $\{p_i(x)\}_{i=0}^{M-1}$, and each $p_i$ parametrized by its respective mean and covariance $(\mu_i, \Sigma_i)$

    \item Mixture of Gaussians (MOGs) in two dimensions for $p_0$ and $p_1$, while $p_2$ is a multivariate Gaussian covering a single mode.
\end{enumerate} 

% We also extend the Gaussian case to higher dimensions to evaluate the feasibility of ACV-SVGD in those settings.

The associated cost vector is given by $w=[1.0, 0.02, 0.04]$.

% \subsection{Estimator Performance for Monte-Carlo Integration}
% \subsubsection{Variance Underestimation with Increasing Dimensionality}




\subsection{Bayesian Inverse Problems via ACV-SVGD}
...
Next, we apply ACV-SVGD to a Bayesian inverse problem. In scientific applications, these involve the inference of unknown parameters $\mathbf{x} \in \RR^d$ given noisy observations. Let $f_i$, $i=0, \cdots M$ denote an ensemble of forward models mapping from identical parametrization $\mathbf{x}$ to simulation output $G$. The data-generating process can be expressed as:
...
Let $\pi_0$ denote the prior distribution on $\mathbf{x}$ and consider zero-mean Gaussian noise with covariance $\Gamma$. Using a common prior specification for each process in Eq.~\eqref{eq: bip_fm}, the posterior distribution is given by Bayes' rule:

\begin{equation}
    p_i(\mathbf{x}) = \frac{1}{Z} L_i \pi_0(\mathbf{x}) = \frac{1}{Z} \exp \left(-\frac{1}{2}||y - f_i(\mathbf{x})||_{\Gamma^{-1}}^2\right)\pi_0(\mathbf{x})
    \label{eq: post}
\end{equation}

where $Z$ is the normalizing constant, typically intractableyou in most problems of interest and $L_i$ is the likelihood corresponding to model $f_i$. When applying SVGD, the $Z$ term drops out because of the use of the score function:
\begin{align}
    \nabla_{x} \log p_i(\mathbf{x}) = \nabla_x (L_i  + \log(\pi_0(\mathbf{x})),
    \label{eqn: bayes_rule_score}
\end{align}

We consider steady-state diffusion based on the setup in \cite{alsup_multilevel_2022}, with $d=2$. The goal is to obtain a posterior distribution on the log-diffusivity coefficient $a(x, \theta)$ given observations of the solution field, and each forward model solved with 1024, 64 and 16 grid points respectively.

    % \item A Darcy flow example based on the setup of \cite{takamoto_pdebench_2024}, previously studied in \cite{coons_bayesian_2025} with $d=30$, and the low-fidelity models based on pre-trained neural network architectures.
% \end{enumerate}
...
In this work, we considered the application of 
multifidelity surrogate and 
multifidelity estimation based on approximate control variates to Stein Variational Gradient Descent (SVGD), a popular Bayesian inference method. This establishes an unbiased estimate of the gradient at each SVGD iteration, formed by propagating subsets of particles under low-fidelity target densities. We learn the control variate weights and a sample allocation strategy to distribute particles amongst various fidelities, which serve as hyperparameters to the ACV estimator. Combined with randomization of the designated high-fidelity subsets at each step, the method can propagate far more particles than its single-fidelity version under identical budgets. It results in modest gains in accuracy when estimating posterior statistics, and returns fairly robust estimates over different replications under a given initialization. These results are sensitive to the choice of initialization too, 

Several promising directions exist for future work. First, it is vital to further enhance accuracy and cost savings from the ACV-based method. To this end, the offline pilot sampling procedure can be refined further and replaced altogether by a combined pilot sampling and estimation step. For instance, the particle ensemble derived from the ACV-estimate at the previous iteration can be exploited to update the confining force covariances, and dynamically update the ACV hyperparameters to exploit variance reduction fully. Additionally, this would overcome the use of potentially sub-optimal ACV-SVGD step size, number of iterations and other hyperparameters which are currently limited by the offline pilot settings. Second, adapting the multi-fidelity procedure to projection and sparsification methods like pSVGD \citep{peng2020} and ccSVGD\citep{padmanabha_csvgd_2025} would unlock further applications of the method to Bayesian Neural Networks and other complex models.  Finally, leveraging the theory and practice of stochastic approximations such as virtual particle methods\cite{das2023} can enable fine-grained allocation strategies and computationally inexpensive ACV updates per iteration as the number of particles scale.
