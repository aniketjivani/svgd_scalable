...
where $\psi^{(j)}(x_i^{(l)}) = [\psi_0(z_j^{(i, l)}, x_i^{(l)}), \cdots, \psi_M(z_j^{(i, l)}, x_i^{(l)})]$ \textcolor{red}{fill in remaining notation}. Thus, every iteration $l$ requires optimization of hyperparameters for multiple outputs, each a confining force update. 
...
We follow the strategy outlined in \cite{Quaglino2019} that chooses a common sample allocation for all particles per dimension $d$ $\CA^d$ based on the scalarized objective:
\begin{align}
...
\end{align}
where $w$ is the normalized cost vector ($w_0=1$) representing update cost of a single particle per iteration under each fidelity. While this is by far the most scalable approach, there is also potential to exploit the correlations between gradient components for even greater variance reduction as done in the multi-output estimator formulation of \citep{dixon_covariance_2024}.

Once $\CA$ is optimized, the optimal control variate weights can be customized per particle (and potentially per dimension) to form the ACV estimator:
...
Next, we compute $\psi_m(z_j^{(i,l)}, \cdot)$ i.e. individual contributions to the confining force under low-fidelity target densities for the same sequence.
