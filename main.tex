...
\end{align}
Here, $\nabla_x \log p(x)$ is the gradient of the log-PDF of the target distribution. In the generative modeling community, this is popularly referred to as the score function or simply the score, which we adopt for the remainder of this text.
...
The first term acts as a confining force drives the particles towards regions towards the high probability regions of the posterior distribution. This primarily depends on the score term which is smoothed by the product with the kernel evaluation. 
The second term acts as a repulsive force that prevents mode collapse and drives particles apart at each SVGD iteration, by an amount depending on the kernel lengthscale, which can be adapted iteration to iteration and the previous particle positions. 
...
While Equation~\ref{eq: vanilla_update} is commonly codified as the update equation for the SVGD algorithm (explicit Euler-like), we need not follow this in practice, and leverage more efficient update procedures. As noted in \cite{wang_stein_2019}, convergence to the target density / fixed point of the system is decoupled from the update procedure itself, which only requires the perturbation direction to be aligned with the action of the Langevin-Stein operator. 
...
However, we leave the integration of efficient kernel approximations for future work. The focus of this work remains on reducing the score function evaluation bottleneck. This will be done through a multi-fidelity setup, leveraging Approximate Control Variates (ACVs) in particular for efficient representation of the confining force.
...
While these represent the first significant efforts targeting more efficient SVGD approximation, we point out some limitations in their flexibility and overall performance:
\begin{enumerate}
    \item Rely only on discretization for forward model operator, but cannot directly leverage models of dissimilar parametrizations, or non-model-based approximations, for example, a mean-field VI based posterior serving as the low-fidelity model.
