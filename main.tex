...
However, like most multi-level analyses, it is somewhat restrictive in its assumptions on the cost and model error relationships, and may not be able to accomodate measure sequences derived from arbitrary data-driven models that may be available to the practitioner. There is no explicit comparison between subsets of lower-fidelity levels either, with all available levels being used irrespective of their potential for cost savings. 
...
where
\begin{align}
    \hat{\psi}_0(x_i^{(l)}, z^{(i, l)}) = \frac{1}{N}\sum_{j=1}^N k(z_j^{(i, l)}, x_i^{(l)}) \nabla_{z_j^{(i, l)}}\log p_0(z_j^{(i, l)})
    \label{eq: cf_expression_hifi}
\end{align}
and
\begin{align}
    \hat{\tau}_0(x_i^{(l)}, z^{(i, l)}) = \frac{1}{N}\sum_{j=1}^N \nabla_{z_j^{(i, l)}} k(z_j^{(i, l)}, x_i^{(l)})
...
\begin{align}
    x_i^{l + 1} &\leftarrow x_i^l + \epsilon_l \textcolor{orange}{\tilde{\psi}} + \epsilon_l \hat{\tau_0}
...
Next, we discuss the selection of hyperparameters for Equation~\eqref{eq: cf_update_acv_formula}.

\subsection{Selection of ACV-SVGD Hyperparameters}

For each particle of the ensemble, the optimal values of the ACV hyperparameters $\alpha$ and $\CA$ depend on the computational costs and the model output covariance matrix $C$ for the vector of CF outputs $\psi(x_i^l, z_i^l) = [\psi_0(x_i^l, z_i^l), \cdots, \psi_M (x_i^l, z_i^l]$. Typically, we do not have access to the true output covariance, and instead must estimate it through pilot sampling. 

For ACV-SVGD, we estimate $C$ via an offline pilot sampling step, where an independent set of pilot particles is first propagated to the high-fidelity target density, and the CF is computed for the resulting particle trajectory under lower-fidelity targets.
...
We first demonstrate the superior performance of the algorithm compared to its single-fidelity counterpart as measured with both KSD and the regularized OT distance between samples at final iteration $L$ and high-quality samples generated from the target dataset.
