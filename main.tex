...
The key contribution is a nested multifidelity update that first partitions the kernel evaluations based on the neighbourhood of influence for each particle, followed by a second partitioning for evaluation under different likelihood models. 
These advancements greatly accelerate Bayesian inference via SVGD and facilitate stochastic optimization for expensive high-fidelity models in particular.
\end{abstract}
...
Bayesian inference is a rigorous and general framework for reasoning under uncertainty: we construct a full probability model for all observable and unobservable quantities in a problem, followed by conditioning on the observed data to characterize the posterior distribution for the unobserved quantities. 

Markov Chain Monte Carlo (MCMC) methods are considered a workhorse of modern statistical inference. 
These work by generating a sequence of random samples through construction of a Markov chain whose equilibrium distribution is the target posterior distribution. 
...
However, these are computationally intensive when dealing with high-dimensional parameter spaces, despite impressive algorithmic advancements that make use of gradient information to guide new proposals.

Variational Inference (VI) reformulates traditional Bayesian inference as an optimization problem, where the posterior is approximated by a parametric family of distributions that is closest to the true posterior through a suitable measure, typically the reverse KL divergence. While VI is much more scalable to higher-dimensional spaces compared to MCMC, its approximation capacity can often be constrained by the choice of the variational family, which may be non-trivial to pin down. 


Stein Variational Gradient Descent (SVGD), introduced in \cite{Liu2016}, proposes a powerful VI method. First, a transport map $T$ is defined between a tractable reference distribution $q_0(x)$ and the target density. 
Rather than restricting the set of transforms $T$ through a certain parametric form, it is instead applied incrementally to propagate an initial set of particles drawn from $q_0$, with each transform directed towards steepest rate of change of KL divergence. Here, the connection between the Langevin-Stein operator and the KL divergence metric is exploited to result in an interpretable and simple inference procedure. While this is of course, sensitive to choices of initial conditions and optimization methods used to find the fixed point, 
...
The kernel weighting factor as well as the particle repulsion term also have scope for cost savings through identification of special covariance structures as the map is iteratively constructed. One simple example is propagating an initial set of particles towards a bimodal target density. 

Eventually, particles will assume memberships of separate clusters such that a particle belonging to one cluster will exhibit little to no influence on those belonging to the other cluster. 

This introduces some structure and sparsity in the covariance matrix. 
...
To reduce the estimator variance, multi-fidelity methods leverage an ensemble of low-fidelity models whose outputs are correlated with the high-fidelity model of interest but with diminished costs. The approximate control variate (ACV) technique \cite{Gorodetsky2020, bomarito_optimization_2022} is one such method that has been shown to produce impressive variance reduction in a variety of scenarios. This method introduces $M$ auxiliary random variables $Q_{m}$ for $m=1,\ldots,M$ that are outputs of the corresponding (e.g., low-fidelity) models $Q_{m} =f_{m}(Z)$, then combines the corresponding MC estimates of each low-fidelity model, $\hat{Q}_{m}$, into a single estimator. The ACV estimator, which we denote $\tilde{Q}$, can be written as:
\begin{align}
        \Tilde{Q}(z,\alpha,\CA) &:= 
...
We also briefly note that there are alternative sampling-based estimators beyond the ACV formulation above. The multilevel best linear unbiased (MLBLUE) estimator from \cite{schaden_multilevel_2020} uses an linear-matrix formulation that 
%also uses a linear combination of correlated MC estimators and 
can produce impressive variance reduction like ACV. More recently, the generalized linear grouped ACV estimator (GACV), the most general multi-fidelity estimator to date, was introduced in \cite{gorodetsky_grouped_2024}. This estimator consists of a linear combination of unbiased estimators, no longer limited to MC estimators, and ACV and MLBLUE can be shown to be special cases of GACV. All such multi-fidelity sampling-based estimators are similarly parameterized by a vector of weights and a sample allocation, although these exact parameterizations and their dimensions may differ across estimators. While MLBLUE and GACV are competitive with and can outperform ACV in some cases, we nonetheless use ACV in this work since potential performance improvements may be marginal relative to the issue of optimal pilot sampling. Importantly, the issue of covariance estimation still applies to these other estimators, and the pilot sampling termination algorithm presented in this paper can be applied to these other estimators by replacing the above variance formulae with their corresponding equations from \cite{gorodetsky_grouped_2024}.
