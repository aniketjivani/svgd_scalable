...
We let $\mathbf{x}^{(l)} = [x_1^{(l)}, \cdots , x_N^{l}]$ denote positions of $N$ particles used in the SVGD approximation at iteration $l$. First, by linearity of expectation, we partition the overall gradient $\hat{\phi}^{\ast}$. Equation~\eqref{eq: vanilla_update} becomes:
\begin{align}
...
An update for particle $x_i^{l}$ involves evaluation of the CF and RF terms at $x_i^l$ as well as its neighbours $z_i^l \triangleq \mathbf{x}^{(l)} \setminus x_i^l$. The repulsive force computation, which depends only on positions of particle pair $(x_i^{(l)}, x_j^{(l)})$, is not affected by the choice of model and therefore does not admit a control-variate representation based on the score function. The SVGD update for the individual models / densities thus features \emph{identical} contributions from the repulsive force. On the other hand, we can replace the confining force by its ACV representation.  Using $\psi$ as shorthand to denote any appearance of CF, we propose:
\begin{align}
