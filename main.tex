...
An example is Stein Variational Gradient Descent (SVGD), a non-parametric variational inference technique which deterministically transports a particle ensemble to cover and approximate complex posterior distributions without restriction to a particular variational family. 
However, the use of SVGD is hampered by the cost of evaluation of target densities based on computationally expensive high-fidelity forward models, which are ubiquitous in scientific applications.
In this work, we contribute to the growing body of work on scalable SVGD approximations by making use of multi-fidelity estimation techniques. Specifically, we use an unbiased estimator of the SVGD confining force term that leverages inexpensive score evaluations derived from lower fidelity forward models or density approximations. Through this flexible setup, a greater number of particles can be propagated for the same computational budget. This results in improved estimates for Monte-Carlo integrands and posterior summary statistics in Bayesian Inference, and is a useful addition to techniques that adapt control variates for accelerating Bayesian inference.
\end{abstract}
...
\aj{add to literature: Support points (VR Joseph) and Stein Points, VP and GB- SVGD (for convergence), thinning for MCMC \url{https://arxiv.org/pdf/2005.03952}, actual multi-level approximation - \url{https://arxiv.org/pdf/2402.01320}}
