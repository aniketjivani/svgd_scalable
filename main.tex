...
Our initial focus is on efficient evaluation of the score function through multi-fidelity setups that reduce the number of high-fidelity model-based likelihood evaluations, before considering a potential combination of the kernel and score approximation.
...
Linear transformations of the score function have been one of the popular choices given the high canonical correlations with the target that result in better variance reduction, and their straightforward incorporation into modern MCMC methods that leverage gradient information \citep{papamarkou_zero_2014}. \cite{si_scalable_2021} extends CV identification to the high-dimensional settings by proposing a variational formulation based on Stein operators \footnote{\textcolor{red}{Technically these are control functionals rather than control variates? \citep{oates_control_2017}}}.
...
\begin{align}
 L_i = \log p(\mathbf{y}_i | \theta) = \prod_{k=1}^N \log p(y_k | \theta)
 \label{eq: lik_fid}
\end{align}

where $\mathbf{y}_i = [y_1, \cdots, y_k, \cdots, y_N]$ are i.i.d. observations at a single realization of $\theta$.

The score function for the target density then simplifies to the following computable expression even when the normalizing constant is not, which permits dealing with unnormalized densities:
...
Most of our initial demonstrations do not specify an explicit prior and likelihood but showcase the methodology on an ensemble of closed-form target density functions. We show an example of mapping from an initial set of particles to a bimodal high-fidelity target density, leveraging a lower-fidelity density $p_1$ which corresponds to one of the modes.
\begin{align}
...
Theory for the weak convergence of KSD is developed in \cite{gorham_measuring_2017}, and suggests that commonly used KSD formulations fail to detect non-convergence for violating sample sequences, even for simple Gaussian targets in small to modest dimensions ($d \geq 3$). We follow their recommendation to use the Inverse Multiquadric (IMQ) Kernel i.e. $k(x, x') = (c^2 + ||x - x'||_2^2)^\beta$ instead of the RBF or Matern kernel for all experiments involving SVGD and corresponding KSD computation, with $\beta=-1/2$ and $c=1$. A comparison of the IMQ and RBF kernel basis functions is shown in \autoref{fig:imq_vs_rbf}, with the RBF kernel defined as $k(x, x') = \exp(-||x - x'||_2^2 / h^2)$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/imq_vs_rbf_basis functions.png}
...
KSD is reported for target ensembles with number of particles $\{16, 32, 64, 128, 256, 512\}$ to measure the gap between the density induced by SVGD particles at the final update step and samples from the true posterior. 
We overlay this trend for the bifidelity case too, with each additional line plot representing the number of particles used in the correction term.
...
First, we establish correlations between the Stein update function values for multiple models over the SVGD trajectory.
...
In this work, we considered the application of multifidelity surrogate and multifidelity estimator techniques to a general -purpose non-parametric variational inference technique, namely the SVGD. SVGD is based on Stein's identity, which has found applications in hypothesis testing, development of traditional control variates for MCMC, hyperparameter marginalization. SVGD in particular is useful for Bayesian inference in settings with access to a smooth and differentiable posterior density. We explore the benefits of approximate control variates based on lower-fidelity likelihood ensembles on the convergence and computational cost of SVGD.
...
This material is based upon work supported in part by the National Science Foundation Graduate Research Fellowship under Grant No. DGE 1841052.

This research is supported in part through computational resources and services provided by Advanced Research Computing at the University of Michigan, Ann Arbor.
