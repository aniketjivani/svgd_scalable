...
Here, the connection between the Langevin-Stein operator and the KL divergence metric is exploited to result in an interpretable and simple inference procedure resembling gradient descent. Notably, the convergence to the target density / fixed point of the system is decoupled from the update procedure, and various strategies can be used to accelerate the optimization.
...
Eventually, particles will assume memberships of separate clusters such that a particle belonging to one cluster will exhibit little to no influence on those belonging to the other cluster. This introduces some structure and sparsity in the covariance matrix. 
...
can produce impressive variance reduction like ACV. More recently, the generalized linear grouped ACV estimator (GACV), the most general multi-fidelity estimator to date, was introduced in \cite{gorodetsky_grouped_2024}. 
This estimator consists of a linear combination of unbiased estimators, no longer limited to MC estimators, and ACV and MLBLUE can be shown to be special cases of GACV. All such multi-fidelity sampling-based estimators are similarly parameterized by a vector of weights and a sample allocation, although these exact parameterizations and their dimensions may differ across estimators. 
...
We begin with the assumption that we have access to a black-box model ensemble $f_i(Z)$, $i=1, \cdots, M$ where the subscript indexes models from the highest to the lowest fidelity, and $Z \in \RR^{n_z}$ is a random vector.
...
\begin{align}
 L_0 &= \log p(y_0 | x, \theta)\\
 L_1 &= \log p(y_1 | x, \theta)
\end{align}
